use crate::return_string_in_buffer;
...
pub unsafe extern "C" fn unit_expression_buf(
    ...
    let unit = &*data;
    return return_string_in_buffer(unit.expression(), buffer, length);
}
...
pub unsafe extern "C" fn unit_composition_buf(
    ...
    let unit = &*data;
    return return_string_in_buffer(unit.composition().to_string(), buffer, length);
}
